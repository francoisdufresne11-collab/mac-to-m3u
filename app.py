...
SAVE_DIR   = "/home/fd/Bureau/coversion"
...
function sauvegarderConn(){
  ...
  if(!name){ alert("⚠️ Donnez un nom à la connexion !"); document.getElementById("connName").focus(); return; }
  ...
  xhr.onload=function(){
    if(xhr.status===200){
      ...
      if(d.ok){ document.getElementById("connName").value=""; alert("✅ Connexion \""+name+"\" sauvegardée !"); chargerListe(); }
    }
  };
...
function afficherConnexions(data){
  ...
  if(!data||data.length===0){
    list.innerHTML="<div class='conn-empty'>Aucune connexion sauvegardée.<br>Remplissez URL+MAC, donnez un nom et cliquez sur 💾 SAUVEGARDER.</div>";
  ...
  data.forEach(function(item,i){
    ...
    div.innerHTML=
      ...
      "<div class='conn-btns'>"+
        ...
        "<button class='btn-sm' style='background:#b91c1c' onclick='supprimerConn("+i+")'>🗑️ Supprimer</button>"+
      "</div>";
...
function restaurer(){
  ...
  if(!file){ alert("⚠️ Sélectionnez un fichier connexions.json !"); return; }
  var fd=new FormData();
  fd.append("file",file);
  var xhr=new XMLHttpRequest();
  ...
  xhr.onload=function(){
    if(xhr.status===200){
      try{
        ...
        if(d.ok){
          msg.innerHTML="<div class='ok'>✅ "+d.count+" connexion(s) restaurée(s) !</div>";
          chargerListe();
          document.getElementById("restoreFile").value="";
        }else{
          msg.innerHTML="<div class='err'>❌ "+d.msg+"</div>";
        }
      }catch(e){}
    }
  };
        ...
        if(d.done){
          ...
          if(d.error){
            ...
            document.getElementById("pRes").innerHTML=
              "<div class='ok'>✅ Conversion réussie !<br>"+
              "📺 Chaînes : <b>"+d.channels+"</b><br>"+
              "📂 Catégories : <b>"+d.categories+"</b><br>"+
              "💾 Taille : <b>"+d.size_kb+" Ko</b><br>"+
              "📁 "+d.filepath+"</div>"+
              "<div class='stats-row'>"+
...
# Route discrete — non visible sur le site
@app.route("/admin/log")
def admin_log():
...
if __name__=="__main__":
    ...
    if not os.path.exists(LOG_FILE):
        open(LOG_FILE,"w",encoding="utf-8").write("=== Historique des connexions MAC to M3U ===\n")
    print("\n"+"="*55)
    print("📁 Script     : %s"%os.path.abspath(__file__))
    print("💾 Connexions : %s"%SAVE_FILE)
    print("📜 Historique : %s"%LOG_FILE)
