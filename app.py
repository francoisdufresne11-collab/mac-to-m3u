    ...
    for path in PATHS:
        ...
        for attempt in range(3):
            try:
                ...
                print("  SSL err attempt %d: %s"%(attempt+1,str(e)))
                time.sleep(1)
            except requests.exceptions.ConnectionError as e:
                print("  Conn err attempt %d: %s"%(attempt+1,str(e)))
                time.sleep(2)
            except requests.exceptions.Timeout as e:
                print("  Timeout attempt %d"%(attempt+1))
                time.sleep(2)
            except Exception as e:
                print("  Err attempt %d: %s"%(attempt+1,str(e)))
                time.sleep(1)
    return None,None
...
<div class="box">
  ...
  <div class="info">ℹ️ Entrez l'URL complète avec le port. Ex: http://example.com:8080</div>
  ...
  <button class="btn btn-blue" id="btnTest" onclick="testerConn()">🔍 Tester la connexion</button>
  <button class="btn btn-blue" id="btnLoad" onclick="chargerCats()" style="margin-top:6px">📂 Charger les catégories</button>
  <div id="testRes"></div>
...
function testerConn(){
  ...
  document.getElementById("btnTest").textContent="⏳ Test en cours...";
  ...
  xhr.onload=function(){
    ...
    document.getElementById("btnTest").textContent="🔍 Tester la connexion";
    if(xhr.status===200){
      try{
        ...
        if(d.ok){
          document.getElementById("testRes").innerHTML="<div class='ok'>✅ Connexion réussie ! Token obtenu.</div>";
        }else{
...
def test_conn():
    ...
    if tok:
        ...
        return Response(json.dumps({"ok":False,"msg":"Connexion impossible. Verifiez URL et MAC.","log":log_output}), mimetype="application/json")
...
def get_cats_route():
    ...
    if not tok:
        ...
        return Response(json.dumps({"error":"Connexion impossible. Verifiez URL et MAC."}),mimetype="application/json")
