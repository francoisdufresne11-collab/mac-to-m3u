...
def log_connexion(portal, mac, succes=True, detail=""):
    os.makedirs(SAVE_DIR, exist_ok=True)
    now = datetime.now().strftime("%d/%m/%Y %H:%M:%S")
    statut = "SUCCES" if succes else "ECHEC"
    ligne = "[%s] %s | URL: %s | MAC: %s | %s\n" % (now, statut, portal, mac, detail)
    try:
        with open(LOG_FILE, "a", encoding="utf-8") as f:
            f.write(ligne)
    except: pass
    print("LOG: "+ligne.strip())
...
def new_tok(portal, mac):
    ...
    print("Base URL: "+base)
    for path in PATHS:
        if CANCEL["flag"]: return None,None
        url=base+path
        print("Essai: "+url)
        for attempt in range(3):
            if CANCEL["flag"]: return None,None
            try:
                r=requests.get(url,headers=mh(mac),
                    params={"type":"stb","action":"handshake","token":"","prehash":"0","JsHttpRequest":"1-xml"},
                    timeout=30, verify=False)
                print("  HTTP %d | len=%d"%(r.status_code, len(r.text)))
                if r.status_code==200 and r.text:
                    try:
                        ...
                        if t:
                            ...
                            print("  Pas de token: "+r.text[:100])
                    except Exception as e:
                        print("  JSON err: "+str(e)+" | "+r.text[:100])
            except requests.exceptions.SSLError as e:
                print("  SSL err attempt %d: %s"%(attempt+1,str(e)[:60]))
                time.sleep(1)
            except requests.exceptions.ConnectionError as e:
                print("  Conn err attempt %d: %s"%(attempt+1,str(e)[:60]))
                time.sleep(2)
            except requests.exceptions.Timeout:
                print("  Timeout attempt %d"%(attempt+1))
                time.sleep(2)
            except Exception as e:
                print("  Err attempt %d: %s"%(attempt+1,str(e)[:60]))
                time.sleep(1)
    return None,None
...
def do_prof(url,mac,tok):
    ...
    try:
        requests.get(url,headers=h,params={
            "type":"stb","action":"get_profile","hd":"1","num_banks":"2",
            "sn":"0000000000000","stb_type":"MAG250","image_version":"218",
            "video_out":"hdmi","device_id":"0000000000000","device_id2":"0000000000000",
            "signature":"","auth_second_step":"1","hw_version":"1.7-BD-00",
            "not_valid_token":"0","client_type":"STB","hw_arch":"mipsel",
            "JsHttpRequest":"1-xml"},timeout=20,verify=False)
    except: pass
...
def get_cats(url,mac,tok):
    ...
    for action in ["get_genres","get_categories","get_itv_genres"]:
        ...
        try:
            ...
            print("get_cats err: "+str(e))
    return []

def req_page(portal,mac,tok,url,genre_id,pg):
    if CANCEL["flag"]: return {},tok,url
    for attempt in range(4):
        ...
        try:
            ...
            r=requests.get(url,headers=h,params={
                ...
                "genre":str(genre_id),"force_ch_link_check":"0",
                "fav":"0","sortby":"number","hd":"0",
                "p":str(pg),"JsHttpRequest":"1-xml"},
                timeout=30,verify=False)
        ...
        except Exception as e: print("req_page err %d: %s"%(attempt+1,str(e)));time.sleep(2)
    return {},tok,url
...
def count_all_pages(portal,mac,tok,url,genre_id,genre_name):
    if CANCEL["flag"]: return 0,tok,url
    ...
    if d:
        ...
        if data:
            count=len(data);pg=1
            while pg<500:
                if CANCEL["flag"]: return count,tok,url
...
def do_convert(portal,mac,selected_cats):
    ...
    time.sleep(0.3)
    try:
        upd(5,"Connexion au portail...")
        tok,lu=new_tok(portal,mac)
        if CANCEL["flag"]:
            G["error"]="Annule par l'utilisateur.";G["done"]=True;G["running"]=False;return
        if not tok:
            log_connexion(portal,mac,succes=False,detail="Token impossible")
            G["error"]="Connexion impossible. Verifiez URL et MAC."
            G["done"]=True;G["running"]=False;return
        log_connexion(portal,mac,succes=True,detail="Token OK")
        upd(10,"Profil...");do_prof(lu,mac,tok)
        if CANCEL["flag"]:
            G["error"]="Annule.";G["done"]=True;G["running"]=False;return
        ...
        if not all_channels:
            G["error"]="Aucune chaine trouvee."
            G["done"]=True;G["running"]=False;return
        upd(87,"%d chaines ! M3U..."%total)
        m3u=build_m3u(all_channels)
        upd(95,"Sauvegarde...")
        ...
        G["channels"]=total;G["categories"]=total_cats
        G["size_kb"]=sk;G["filepath"]=fp
        G["done"]=True;G["running"]=False
        print("FINI: %d chaines — %s Ko"%(total,str(sk)))
    except Exception as e:
        import traceback;traceback.print_exc()
        G["error"]=str(e);G["done"]=True;G["running"]=False

def do_get_cats(portal,mac):
    global G
    CANCEL["flag"]=False
    ...
    if CANCEL["flag"]:
        G["msg"]="Annule.";G["running"]=False;G["done"]=True
        G["cats"]=[];G["cancelled"]=True;return
    if not tok:
        log_connexion(portal,mac,succes=False,detail="get_cats echec")
        G["running"]=False;G["done"]=True
        G["error"]="Connexion impossible. Verifiez URL et MAC.";G["cats"]=[];return
    log_connexion(portal,mac,succes=True,detail="get_cats OK")
    ...
    if not cats:
        G["running"]=False;G["done"]=True
        G["error"]="Aucune categorie trouvee.";G["cats"]=[];return
    total_c=len(cats);result=[]
    for i,c in enumerate(cats):
        if CANCEL["flag"]:
            G["msg"]="Annule.";G["running"]=False;G["done"]=True
            G["cats"]=result;G["cancelled"]=True;return
    ...
    G["running"]=False;G["done"]=True
    G["cats"]=result;G["cancelled"]=False
...
.warn{margin-top:12px;padding:13px;border-radius:10px;background:rgba(255,153,0,0.1);border:1px solid #f0883e;color:#f0883e;font-size:13px}
.info{margin-top:8px;padding:10px;border-radius:8px;background:rgba(0,212,255,0.08);border:1px solid #00d4ff;color:#00d4ff;font-size:12px}
...
.prog-fill.warn{background:#c47a00}
...
.log-box{background:#0d1117;border-radius:8px;padding:10px;font-family:monospace;font-size:11px;max-height:150px;overflow-y:auto;border:1px solid #30363d;margin-top:8px;white-space:pre-wrap;color:#8b949e}
...
<div class="box">
  ...
  <div class="info">ℹ️ URL complète avec port. Ex: http://example.com:8080</div>
  ...
  <div class="btn-row">
    <button class="btn btn-blue" id="btnTest" onclick="testerConn()">🔍 Tester</button>
    <button class="btn btn-blue" id="btnLoad" onclick="chargerCats()">📂 Charger les catégories</button>
  </div>
  <button class="btn btn-red" id="btnCancel" onclick="annuler()" style="display:none">⛔ Annuler la connexion</button>

  <div id="testRes"></div>
  <div class="log-box" id="testLog" style="display:none"></div>
</div>

<!-- SAUVEGARDER -->
<div class="box" style="border:2px solid #d4a017">
  <div class="box-title" style="color:#d4a017">💾 Sauvegarder cette connexion</div>
  <div class="info">⚠️ Sur Render les sauvegardes sont perdues au redémarrage. Téléchargez connexions.json !</div>
...
<div class="box" style="border:2px solid #00d4ff">
  ...
  <div class="dl-row">
    <a href="/dl_script" class="dl-btn btn-purple">⬇️ Télécharger<br>app.py</a>
...
<div class="box" id="boxCats" style="display:none">
  ...
  <div class="btn-row" style="margin-top:12px">
    <button class="btn btn-purple" id="btnConv" onclick="convertirSel()">🚀 Convertir sélection</button>
    <button class="btn btn-green" onclick="convertirTout()">🌍 Tout convertir</button>
  </div>
  <button class="btn btn-red" id="btnCancelConv" onclick="annuler()" style="display:none;margin-top:8px">⛔ Annuler la conversion</button>
</div>
...
function setBusy(b){
  BUSY=b;
  document.getElementById("btnTest").disabled=b;
  document.getElementById("btnLoad").disabled=b;
  document.getElementById("btnCancel").style.display=b?"block":"none";
  if(document.getElementById("btnConv"))
    document.getElementById("btnConv").disabled=b;
  if(document.getElementById("btnCancelConv"))
    document.getElementById("btnCancelConv").style.display=b?"block":"none";
}
...
function annuler(){
  ...
  xhr.onload=function(){
    ...
    document.getElementById("btnTest").textContent="🔍 Tester";
    if(document.getElementById("btnConv"))
      document.getElementById("btnConv").textContent="🚀 Convertir sélection";
    setBar(0,"Annulé","Connexion annulée par l'utilisateur.","warn");
    document.getElementById("testRes").innerHTML="<div class='warn'>⛔ Annulé !</div>";
    if(document.getElementById("pRes"))
      document.getElementById("pRes").innerHTML="<div class='warn'>⛔ Annulé par l'utilisateur.</div>";
  };
  xhr.send();
}
  ...
  xhr.onload=function(){
    ...
    if(xhr.status===200){
      try{
        ...
        if(d.ok){
          document.getElementById("testRes").innerHTML="<div class='ok'>✅ Connexion réussie !</div>";
        }else{
          document.getElementById("testRes").innerHTML="<div class='err'>❌ "+d.msg+"</div>";
        }
      }catch(e){ document.getElementById("testRes").innerHTML="<div class='err'>❌ Erreur: "+e.message+"</div>"; }
    }
  };
  xhr.onerror=function(){
    setBusy(false);
    document.getElementById("btnTest").textContent="🔍 Tester";
    document.getElementById("testRes").innerHTML="<div class='err'>❌ Erreur réseau</div>";
  };
  ...
  xhr.onload=function(){
    if(xhr.status===200){
      try{
        ...
        if(d.ok){ msg.innerHTML="<div class='ok'>✅ "+d.count+" connexion(s) restaurée(s) !</div>"; chargerListe(); }
...
function chargerCats(){
  ...
  document.getElementById("pRes")||(document.getElementById("pRes") && (document.getElementById("pRes").innerHTML=""));
  setBar(2,"Connexion...","Chargement des catégories...","");
  startPoll(function(d){
    document.getElementById("btnLoad").textContent="📂 Charger les catégories";
    if(d.cancelled){
      setBar(0,"Annulé","Annulé par l'utilisateur.","warn");
      return;
    }
    if(d.error){ setBar(0,"Erreur",d.error,"err"); return; }
...
function lancerConv(ids){
  ...
  setBusy(true);
  document.getElementById("btnConv").textContent="⏳ Conversion...";
  setBar(1,"Démarrage...","Préparation...","");
  document.getElementById("boxProg").scrollIntoView({behavior:"smooth"});
  startPoll(function(d){
    document.getElementById("btnConv").textContent="🚀 Convertir sélection";
    if(d.cancelled){
      document.getElementById("pRes").innerHTML="<div class='warn'>⛔ Conversion annulée par l'utilisateur.</div>";
      return;
    }
    if(d.error){
...
def test_conn():
    ...
    if tok:
        log_connexion(portal, mac, succes=True, detail="Test OK")
        return Response(json.dumps({"ok":True,"msg":"Token OK","log":log_output}), mimetype="application/json")
    else:
        log_connexion(portal, mac, succes=False, detail="Test ECHEC")
        return Response(json.dumps({"ok":False,"msg":"Connexion impossible.","log":log_output}), mimetype="application/json")
...
def dl_script():
    ...
    return send_file(fp,as_attachment=True,download_name="app.py",mimetype="text/plain")
...
def get_cats_route():
    portal=request.form.get("portal","").strip()
    mac=request.form.get("mac","").strip()
    t=threading.Thread(target=do_get_cats,args=(portal,mac))
